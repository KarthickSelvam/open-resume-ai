...
| **4. Import From Existing Resume** | If you already have an existing resume (PDF, DOCX, or DOC format), you have the option to import it directly using the Unstructured API, so you can update your resume design to a modern professional design in literally a few seconds. |
...
| **Document Parser** | [Unstructured API](https://unstructured.io/) | Unstructured API parses various document formats (PDF, DOCX, DOC) and is used by the resume parser to extract structured content from resumes. |
...
| /resume-import | /resume-import/page.tsx | Resume import page, where you can choose to import data from an existing resume document (PDF, DOCX, DOC). The main component used is `ResumeDropzone` (`/components/ResumeDropzone.tsx`) |
...
To use the resume import and parser features, you need to configure the Unstructured API:

1. Sign up for a free account at [Unstructured.io](https://unstructured.io/)
2. Get your API key from the dashboard
3. Copy `.env.example` to `.env` in the project root
4. Add your API key to the `.env` file:
   ```
   NEXT_PUBLIC_UNSTRUCTURED_API_KEY=your_api_key_here
   ```

**Note:** Without the API key, the resume import and parser features will not work. The resume builder will still function normally.
...
4. Set up your Unstructured API key (see Prerequisites above)
...
1. Download the repo `git clone https://github.com/xitanggg/open-resume.git`
2. Change the directory `cd open-resume`
3. Set up your Unstructured API key (see Prerequisites above)
4. Build the container `docker build -t open-resume .`
5. Start the container `docker run -p 3000:3000 open-resume`
6. Open your browser and visit [http://localhost:3000](http://localhost:3000) to see OpenResume live
